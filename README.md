<div align="center">

<img 
  src="./banner-sutmi.png" 
  width="100%" 
  alt="Sutmi Ramirez - Software Engineer y Frontend Developer"
/>

<br>
<br>

### 👋 Hola, soy Sutmi Ramirez

**Ingeniera en Software · Frontend Developer**

<p>
  Desarrollo interfaces modernas, intuitivas y responsivas,<br>
  enfocadas en simplificar procesos y mejorar la experiencia del usuario.
</p>

</div>

<br>

---

## 👩‍💻 Sobre mí

Soy egresada de **Ingeniería en Software**, enfocada principalmente en el desarrollo Frontend y en la creación y mejora de interfaces para aplicaciones web.

Me gusta transformar procesos complejos en experiencias más claras, organizadas e intuitivas, especialmente dentro de sistemas administrativos y aplicaciones empresariales.

Actualmente trabajo principalmente con **Angular, TypeScript y Firebase**, participando en el desarrollo de módulos, interfaces y flujos relacionados con reservaciones, administración, pagos, facturación y gestión operativa.

<br>

<div align="center">

`💻 Desarrollo Frontend` &nbsp;&nbsp;
`🎨 UI/UX` &nbsp;&nbsp;
`🧩 Sistemas administrativos` &nbsp;&nbsp;
`📱 Diseño responsive`

</div>

<br>

---

## 🛠️ Tecnologías

<div align="center">

### Frontend

<img src="https://skillicons.dev/icons?i=angular,ts,js,html,css" />

<br>
<br>

### Servicios & Herramientas

<img src="https://skillicons.dev/icons?i=firebase,nodejs,git,github,vscode,figma" />

</div>

<br>

---

## 💼 Experiencia profesional

He participado en el desarrollo y mejora de aplicaciones empresariales compuestas por diferentes módulos y procesos administrativos.

<table>

<tr>

<td width="50%" valign="top">

### 🏨 Gestión hotelera

Desarrollo y mejora de interfaces para procesos relacionados con:

- Reservaciones
- Calendarios de ocupación
- Disponibilidad de habitaciones
- Gestión de huéspedes
- Check-in y check-out
- Pagos
- Interfaces administrativas

</td>

<td width="50%" valign="top">

### 🧾 Facturación

Participación en interfaces y flujos relacionados con:

- Gestión de facturas
- Clientes fiscales
- Solicitudes de facturación
- Conceptos
- Pagos
- Flujos administrativos

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 🏢 Sistemas administrativos

Trabajo en aplicaciones conformadas por diferentes módulos:

- Hotel
- Cabañas
- Locales
- Recursos Humanos
- Facturación
- Pagos

</td>

<td width="50%" valign="top">

### 🎨 Frontend & UI

Enfoque en el desarrollo y mejora de:

- Rediseño de interfaces
- Componentes reutilizables
- Diseño responsive
- Dashboards
- Tablas y filtros
- Experiencia de usuario

</td>

</tr>

</table>

<br>

---

## 🚀 Proyectos destacados

<table>

<tr>

<td width="50%" valign="top">

### 🏨 Plataforma de Gestión Hotelera

Sistema administrativo orientado a la gestión y operación de un hotel.

**Mi participación:**

- Interfaces de reservaciones
- Calendarios de ocupación
- Gestión de habitaciones
- Check-in y check-out
- Pagos
- Facturación
- Diseño responsive

<br>

**Tecnologías**

`Angular` `TypeScript` `Firebase` `Firestore`

</td>

<td width="50%" valign="top">

### 🧾 Sistema de Facturación

Plataforma para administrar procesos y solicitudes relacionadas con facturación.

**Mi participación:**

- Gestión de clientes
- Facturas
- Solicitudes
- Conceptos
- Flujos de pago
- Interfaces administrativas

<br>

**Tecnologías**

`Angular` `TypeScript` `Firebase` `REST API`

</td>

</tr>

<tr>

<td width="50%" valign="top">

### 📅 Gestión de Reservaciones

Interfaces enfocadas en consultar disponibilidad, administrar reservaciones y visualizar la ocupación.

**Funcionalidades:**

- Calendario
- Disponibilidad
- Reservaciones múltiples
- Gestión de ocupación
- Interfaces responsive

<br>

**Tecnologías**

`Angular` `TypeScript` `Firebase`

</td>

<td width="50%" valign="top">

### 🌙 Luna Planner

Aplicación enfocada en la organización de notas y productividad personal.

**Funcionalidades:**

- Gestión de notas
- Organización de información
- Interfaz enfocada en productividad

<br>

**Tecnologías**

`Java` `Android`

</td>

</tr>

</table>

<br>

---

## 🌱 Actualmente

Actualmente continúo desarrollando soluciones Frontend y fortaleciendo mis conocimientos en arquitectura de aplicaciones web, diseño de interfaces y construcción de sistemas organizados, mantenibles y escalables.

### Áreas de interés

<div align="center">

`Frontend Development` ·
`Angular` ·
`UI/UX` ·
`Software Engineering`

</div>

<br>

---

## 📫 Contacto

<div align="center">

### ¿Trabajamos juntos?

Estoy abierta a oportunidades profesionales y proyectos relacionados con **desarrollo Frontend, diseño de interfaces y desarrollo de software**.

<br>

<a href="TU_LINKEDIN">
  <img src="https://img.shields.io/badge/LinkedIn-Perfil%20profesional-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="TU_PORTAFOLIO">
  <img src="https://img.shields.io/badge/Portafolio-Ver%20portafolio-181717?style=for-the-badge&logo=googlechrome&logoColor=white" />
</a>

<a href="mailto:TU_CORREO">
  <img src="https://img.shields.io/badge/Email-Contacto-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

<br>
<br>

---

### ✨ Gracias por visitar mi perfil

</div>
